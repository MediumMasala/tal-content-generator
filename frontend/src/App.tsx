...
function App() {
  ...
  // Handle the analysis
  const handleAnalyze = async (url: string, userTopic?: string) => {
    console.log('Analyzing:', url, userTopic ? `with topic: ${userTopic}` : '');
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/generate`, {
        ...
        body: JSON.stringify({ linkedinUrl: url, userTopic }),
      });
          ...
          <AnimatePresence>
            {appState === 'completed' && results && (
              ...
              >
                ...
                {/* Topic focus badge */}
                {results.topicUsed && results.topicUsed !== 'general' && (
                  <div className="flex items-center justify-center mt-3">
                    <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-violet-500/20 border border-violet-500/30">
                      ...
                      <span className="text-violet-300 text-xs font-medium">
                        Focus: {results.topicUsed.replace('_', ' ')}
                      </span>
                    </div>
                  </div>
                )}
